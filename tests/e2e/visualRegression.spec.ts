...
let devotions = range(globals.today - 3, globals.today, 1);
  ...
  test.describe(`${locale} devotions`, () => {
    for (const devotion of devotions) {
      test(`Day ${devotion}`, async ({ page }) => {
        ...
        await expect(page).toHaveScreenshot(`${locale}-classic-${devotion}.png`, {
          fullPage: true,
        });
      });
    }
...
test.describe('Devotionals', () => {
  test(`Devotionals`, async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot(`310-en-classic.png`, {
      fullPage: true,
    });
  });
});
